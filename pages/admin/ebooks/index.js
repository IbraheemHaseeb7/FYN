...
export default function Level1() {
  ...
  async function accept(e, uid, id) {
    ...
    await deleteDoc(doc(firestore, `ebook`, id));

    toast.success("User has now access to ebooks");
  }
  async function remove(e, uid) {
    ...
    toast.success("user has been removed from ebook");
  }
  ...
  useEffect(() => {
    let unsub = onSnapshot(
      ...
      (data) => {
        ...
        setUsers(array);
        setUserCursor(array[array.length - 1]?.ebookid);
  ...
  async function loadMoreUsers(e) {
    ...
    await getDocs(
      ...
      setUserCursor(prevArray[prevArray.length - 1]?.ebookid);
