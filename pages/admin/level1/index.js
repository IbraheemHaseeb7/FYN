...
export default function Level1() {
  ...
  async function accept(e, uid, id) {
    ...
    await deleteDoc(doc(firestore, `level1`, id));

    toast.success("User is now level 1");
  }
  async function remove(e, uid) {
    ...
    toast.success("user has been removed from level 1");
  }
  ...
  useEffect(() => {
    let unsub = onSnapshot(
      ...
      (data) => {
        ...
        setUsers(array);
        setUserCursor(array[array.length - 1].level1id);
  ...
  async function loadMoreUsers(e) {
    ...
    await getDocs(
      ...
      setUserCursor(prevArray[prevArray.length - 1]?.level1id);
